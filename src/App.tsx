...
import { Asset, AssetType, Transaction, Dividend, Broker, AppData, DataSnapshot } from './types';
...
export default function App() {
  ...
  const handleRestoreSnapshot = (snapshot: DataSnapshot) => {
    askConfirm(
      ...
      `Deseja realmente restaurar os dados da versão ${snapshot.version} criada em ${format(parseISO(snapshot.date), 'dd/MM/yyyy HH:mm')}? Os dados atuais serão substituídos.`,
      () => {
        setData(prev => ({
          ...prev,
  ...
  const handleDarfDownload = (data: any) => {
    setSelectedDarf(data);
    setIsDarfModalOpen(true);
  };

  const handlePrintDarf = () => {
    window.print();
  };

  const generateDarfNumber = () => {
  ...
  const handleCreateBroker = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (cnpj && !validateCNPJ(cnpj)) {
      showNotify('CNPJ da corretora inválido!', 'error');
      return;
    }
  ...
  const handleSaveAsset = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (cnpj && !validateCNPJ(cnpj)) {
      showNotify('CNPJ do ativo inválido!', 'error');
      return;
    }
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length === 0 || !currentBroker) return;
    ...
    for (let f = 0; f < files.length; f++) {
      const file = files[f];
      console.log(`Processando arquivo ${f + 1}/${files.length}: ${file.name}`);
      showNotify(`Analisando ${f + 1}/${files.length}: ${file.name}...`);
      ...
      console.log('Início do texto:', fullText.substring(0, 200));
        ...
        if (extractedData && (extractedData.transactions?.length > 0 || extractedData.dividends?.length > 0)) {
          ...
          errors.push(`${file.name}: nenhum dado encontrado`);
        }
      } catch (error: any) {
    ...
    if (errors.length > 0) {
      showNotify(`Concluído com erros: ${errors.join(' | ')}`, 'error');
    } else {
      showNotify(`✅ ${files.length} PDF(s) importados: ${totalTransactions} transações e ${totalDividends} rendimentos.`);
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded text-[10px] font-mono font-bold">v{APP_VERSION}</span>
                </div>
                                ...
                                <td className="py-4 text-center">
                                  {data.totalDarf >= 10 ? (
                                    <div className="flex flex-col items-center gap-2">
                                      ...
                                      <Button size="sm" variant="outline" className="h-7 text-[10px]" onClick={() => handleDarfDownload(data)}>
                                        Gerar DARF
                                      </Button>
                                    </div>
                                  ) : data.totalDarf > 0 ? (
  ...
  {/* Modals */}
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-blue-800 leading-relaxed">
                ...
                Se você estiver usando o aplicativo fora do ambiente de desenvolvimento, precisará de uma chave de API própria.
              </p>
            </div>
          </div>
              ...
              data.snapshots.map(snapshot => (
                <div key={snapshot.id} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-blue-300 transition-colors group">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 flex items-center gap-1">
                        <TrendingUp className="w-3 h-3" /> {format(parseISO(snapshot.date), 'dd/MM/yyyy HH:mm')}
                      </p>
                    </div>
                  ...
                  <div className="flex gap-4 text-[10px] text-slate-500">
                    ...
                    <p><b>{snapshot.data.reduce((acc, b) => acc + (b.assets?.length || 0), 0)}</b> Ativos</p>
                  </div>
                </div>
              ))
            )}
      ...
      <Modal 
        ...
        title="Documento de Arrecadação de Receitas Federais (DARF)"
        footer={
          <>
            <Button variant="ghost" onClick={() => setIsDarfModalOpen(false)}>Fechar</Button>
            <Button onClick={handlePrintDarf} variant="primary">
              <Printer className="w-4 h-4 mr-2" /> Imprimir DARF
            </Button>
          </>
        }
      >
        <div className="bg-white border-2 border-black p-6 font-mono text-[10px] uppercase leading-tight print:block print:fixed print:inset-0 print:m-0 print:p-8 print:z-[100] print:bg-white overflow-y-auto">
          <div className="flex justify-between border-b-2 border-black pb-2 mb-4">
            <div className="flex items-center gap-2">
              <div className="w-12 h-12 border border-black flex items-center justify-center font-bold">RFB</div>
              <div>
                <p className="font-bold text-xs uppercase">Ministério da Fazenda</p>
                <p className="uppercase">Secretaria da Receita Federal do Brasil</p>
              </div>
            </div>
            <div className="text-right">
              <p className="font-bold text-lg">DARF</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-0 border-2 border-black overflow-hidden mb-4">
            <div className="border-r-2 border-b-2 border-black p-2">
              <p className="font-bold mb-1">01 NOME / TELEFONE</p>
              <p>{currentBroker?.name}</p>
              <p>CONTRIBUINTE: {data.brokers[0]?.name} (DEMO)</p>
            </div>
            <div className="border-b-2 border-black p-2 bg-slate-50">
              <p className="font-bold mb-1 underline">ATENÇÃO</p>
              <p className="text-[8px] normal-case leading-none">É vedado o recolhimento de tributos administrados pela RFB cujo valor total seja inferior a R$ 10,00.</p>
            </div>
            <div className="border-r-2 border-b-2 border-black p-2">
              <p className="font-bold mb-1">02 PERÍODO DE APURAÇÃO</p>
              <p className="text-sm font-bold">{selectedDarf ? format(parseISO(`${selectedDarf.month}-01`), 'dd/MM/yyyy') : '-'}</p>
            </div>
            <div className="border-b-2 border-black p-2">
              <p className="font-bold mb-1">03 NÚMERO DO CPF OU CNPJ</p>
              <p className="text-sm font-bold">{currentBroker?.cnpj || '00.000.000/0001-00'}</p>
            </div>
            <div className="border-r-2 border-b-2 border-black p-2">
              <p className="font-bold mb-1">04 CÓDIGO DA RECEITA</p>
              <p className="text-sm font-bold">6015</p>
            </div>
            <div className="border-b-2 border-black p-2">
              <p className="font-bold mb-1">05 NÚMERO DE REFERÊNCIA</p>
              <p>-</p>
            </div>
            <div className="border-r-2 border-b-2 border-black p-2">
              <p className="font-bold mb-1">06 DATA DE VENCIMENTO</p>
              <p className="text-sm font-bold">
                {selectedDarf ? format(endOfMonth(parseISO(`${selectedDarf.month}-01`)), 'dd/MM/yyyy') : '-'}
              </p>
            </div>
            <div className="border-b-2 border-black p-2">
              <p className="font-bold mb-1">07 VALOR DO PRINCIPAL</p>
              <p className="text-sm font-bold text-right">{selectedDarf ? formatCurrency(selectedDarf.totalDarf) : '-'}</p>
            </div>
            <div className="border-r-2 border-b-2 border-black p-2">
              <p className="font-bold mb-1">08 VALOR DA MULTA</p>
              <p className="text-right">0,00</p>
            </div>
            <div className="border-b-2 border-black p-2">
              <p className="font-bold mb-1">09 VALOR DOS JUROS</p>
              <p className="text-right">0,00</p>
            </div>
            <div className="border-r-2 border-black p-2">
              <p className="font-bold mb-1 italic">AUTENTICAÇÃO BANCÁRIA (Somente nas 1ª e 2ª vias)</p>
              <div className="h-8"></div>
            </div>
            <div className="p-2">
              <p className="font-bold mb-1">10 VALOR TOTAL</p>
              <p className="text-sm font-bold text-right">{selectedDarf ? formatCurrency(selectedDarf.totalDarf) : '-'}</p>
            </div>
          </div>

          <div className="text-center mt-6 py-4 border-2 border-black">
            <p className="font-bold">LINHA DIGITÁVEL PARA PAGAMENTO (SIMULADA)</p>
            <p className="text-xs font-bold font-mono tracking-widest mt-2">{generateDarfNumber()} {generateDarfNumber()} {generateDarfNumber()}</p>
          </div>
          
          <div className="mt-8 pt-8 border-t border-dashed border-slate-300 no-print">
            <p className="text-xs text-slate-500 italic lowercase text-center">Este é um documento gerado para fins de auxílio e conferência de valores. Para pagamentos oficiais, utilize os canais autorizados da Receita Federal (e-CAC / SicalcWeb).</p>
          </div>
        </div>
      </Modal>
